    ...
    public partial class Parameters : Resource {
        ...
        public void Store(string fn, int rows, int cols, string id) {
            for(int i=0; i<Matrices.Length; i++) {
                if(Matrices[i] != null) {
                    if(Matrices[i].ID == id) {
                        GD.PrintErr("Matrix with ID " + id + " already contained.");
                        return;
                    }
                }
            }
        ...
        public Matrix Load(string id) {
            ...
            if(matrix == null) {
                GD.PrintErr("Matrix with ID " + id + " not found.");
            }
            return matrix;
        }
        ...
        private Matrix ReadBinary(string fn, int rows, int cols, string id) {
            if(File.Exists(fn)) {
                ...
                reader.Close();
                if(errors > 0) {
                    GD.PrintErr("There were " + errors + " errors reading file at path " + fn + ".");
                    return null;
                } else {
                    return matrix;
                }
            } else {
                GD.PrintErr("File at path " + fn + " does not exist.");
                return null;
            }
        }
