	...
	public class Tensor
	{
		...
		public void SetValue(int row, int col, float value) {
			if(row >= GetRows() || col >= GetCols()) {
				GD.PrintErr("Setting out of bounds at [" + row + ", " + col + "].");
				return;
			}
		...
		public float GetValue(int row, int col) {
			if(row >= GetRows() || col >= GetCols()) {
				GD.PrintErr("Getting out of bounds at [" + row + ", " + col + "].");
				return 0f;
			}
		...
		public static Tensor Add(Tensor lhs, Tensor rhs, Tensor OUT) {
			if(lhs.GetRows() != rhs.GetRows() || lhs.GetCols() != rhs.GetCols()) {
				GD.PrintErr("Incompatible tensor dimensions.");
			} else {
		...
		public float RowMean(int row) {
			if(row >= GetRows()) {
				GD.PrintErr("Accessing out of bounds.");
				return 0f;
			}
